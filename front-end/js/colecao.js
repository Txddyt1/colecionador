...
if (moeda.imagem) {
    let imgUrl;
    if (typeof moeda.imagem === 'string') {
        imgUrl = moeda.imagem; // Caso a imagem seja uma URL
    } else {
        imgUrl = URL.createObjectURL(moeda.imagem); // Caso seja um Blob ou File
    }
...
async function excluirMoeda(dados) {
    ...
    form.addEventListener('submit', async (evento) => {
        ...
        const idUsuario = usuarioLogado?.idUsuario || null; // Adicionando checagem
        const idMoeda = moeda.idMoeda || null; // Usando o idMoeda de forma segura
